<?php 
//число пользователей на сайте в метабоксе "На виду" start 
function wph_count_users() {
    global $wpdb;
    $users = $wpdb->get_var("SELECT COUNT(ID) FROM $wpdb->users");
    echo "<tr><td class='first b'>Всего пользователей сайта: 
    <strong><a href=\"users.php\">{$users}</a></strong></td></tr>";
}
add_action('activity_box_end', 'wph_count_users');
//число пользователей на сайте в метабоксе "На виду" end
...
if( have_posts() ) :
  while( have_posts() ) : the_post();
    ...
    $post_id = get_the_id(); // Так можно получить ID поста
    echo get_post_meta( $post_id, '_my_meta_value_key', true );
    // или короче
